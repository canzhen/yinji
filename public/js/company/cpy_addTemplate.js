...
(function ($) {
	...
	$.fn.fileinput.locales.cn = {
		...
		msgInvalidFileType: '文件类型"{name}"无效. 只支持类型为"{types}"的文件。',
		msgInvalidFileExtension: '文件后缀"{name}"无效。只支持后缀为"{extensions}"的文件。',
		msgValidationError: '文件上传错误',
		msgLoading: '正在上传第{index}个{files} &hellip;',
		msgProgress: '正在上传第{index}个{files} - {name} - 上传{percent}%',
		msgSelected: '选择{n}个 {files}',
		msgFoldersNotAllowed: '只能拖拽文件！ {n}个文件夹被忽略',
		dropZoneTitle: '拖拽文件到此处 &hellip;'
	};


	$.extend($.fn.fileinput.defaults, $.fn.fileinput.locales.cn);
